...
class UUIDModelV2(models.Model):
    ...
    def get_initial_id(cls) -> int:
        """
        Starting point for the auto-generated ``id`` field when the table has
        no rows yet. Defaults to ``cls._INITIAL_ID_``. Override this method
        (instead of only setting ``_INITIAL_ID_``) when the starting id needs
        to be resolved dynamically, e.g. from a query.
        @return: int
        """
        return cls._INITIAL_ID_
    ...
    @classmethod
    def next_id(cls, using=None) -> int:
        """
        Next id to assign. Starts at ``get_initial_id()`` when the table has
        no rows yet, otherwise continues from ``max_id() + 1``.
        @return: int
        """
        current_max = cls.max_id(using=using)

        if current_max == 0:
            return cls.get_initial_id()

        return current_max + 1

    @classmethod
    def _assign_auto_ids(cls, objs, using=None) -> None:
        """
        Assigns sequential auto-ids to objects that lack one, intended for use
        inside a transaction.atomic block in bulk_create so the advisory lock
