...
class ModelBaseMeta(ModelBase):
    ...
    @staticmethod
    def _add_formated_number(model_class, attrs):
        """
        ...
        for _field_name, _field in attrs.items():
            ...
            if type(_field) is cached_property:
                _func = _field.func
                try:
                    _hint = get_type_hints(_func).get('return')
                    _add_method = issubclass(_hint, (int, float))
